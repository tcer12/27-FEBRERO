    ...
    revealBtn.title =
      state.failedAttempts < 3
        ? `Necesitas ${
          ...
          } intentos más para ver la palabra`
        : 'Ver la palabra';
  }
}
...
function handleKeyPress(key) {
  ...
  if (key === '⌫' || key === 'BACKSPACE') {
    ...
    if (state.currentGuess.length === state.wordLength) {
      ...
      showToast('¡Oh! Parece que te faltan letras. ¡Completa la palabra!');
      const grid = document.getElementById('grid');
      grid.classList.add('shake');
      setTimeout(() => grid.classList.remove('shake'), 500);
    }
...
function showStats(won = false) {
  ...
  content.innerHTML = `
    <div class="modal-header">
      <h2 class="modal-title">${won ? '¡Victoria! 🏆' : '¡Inténtalo de nuevo! 🎮'}</h2>
      <p class="modal-subtitle">${won ? '¡Excelente trabajo! Has demostrado tu conocimiento de la cultura dominicana.' : `La palabra era: ${state.currentWordObj.word}`}</p>
    </div>
